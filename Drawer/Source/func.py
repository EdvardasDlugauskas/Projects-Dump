...
def graph(chain, coord=None, center=False, color=black, surface=None):   # Center is not None for now...
    """
    ...
    newPoints = []
    if coord is None:
        # Just center of the screen...
        coord = (surface.get_width()/2, surface.get_height()/2)
    if not center:
        for point in points:
